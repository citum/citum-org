...
The actual migration problem is not straightforward. CSL 1.0 styles are XML
programs. They encode citation and bibliography behavior through nested macros,
conditionals, locale terms, substitution rules, position tests, type tests, and
formatting inheritance. Citum styles are deliberately different: declarative
YAML or JSON, typed components, explicit options, and style inheritance rather
than procedural layout trees.
...
Recent work has focused on keeping that search bounded and explainable. Rather
than generating unbounded combinations of edits, the migrator tries small,
typed candidate families and records which one won. Examples include choosing
between the inferred template and the XML-compiled template, applying
contributor case changes for bibliography output, using a type's local default
template, trying fuller date granularity for media and web-like records, and
suppressing specific article-journal components when the measured output says
that helps.

This is a better shape for the problem. It is measurable, debuggable, and
incremental. It also avoids turning migration into a black-box mutation search.
...
mid 60s. That is meaningful progress. It is also not enough to claim that CSL
migration is solved.
...
That last point matters. Some migration defects are engine bugs or schema gaps,
and those should be fixed in `citum-core`. But many are judgement calls: how to
